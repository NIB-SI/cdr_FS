...
| **no concentration–response question at all** — you want the redundancy and sparsity filtering over the whole feature set | `[select] enabled = false` | The method. `emd`, `fit` and `select` are skipped and every feature carries forward. Outputs are named for the input they were built from — `all_representatives.txt`, `final_all_representatives*`, or `final_all*` with correlation off too — so this chain and the full one can share an output directory. `[design]` is still required and still checked against the data; what goes is the constraint that the models be identifiable, so a three-level design runs |
